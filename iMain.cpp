...
std::vector<PlayerData> player(size);
    ...
    for (int i = 0; i < size - 1; ++i) {
        for (int j = 0; j < size - i - 1; ++j) {
            if (player[j].score > player[j+1].score) {
                ...
                std::string tempName = player[j].name;
                player[j].name = player[j+1].name;
                player[j+1].name = tempName;
            }
        }
    }
}
...
void saveTotalTimeToFile() {
	...
	player[g].name = playerName;
	player[g].score = frame_count/10;
	...
	std::fstream MyFile("fileName.txt", std::ios::out);

	for (int i = 0; i < size; ++i) {
		MyFile << player[i].name << std::endl;
		MyFile << player[i].score << std::endl;
	}
...
void score() {
	...
	/*for (int i = 0; i < size; ++i) {
		if (player[i].score > 0 && player[i].score != 1111){
		std::string texta = std::to_string(static_cast<long long>(i+1)) + "	 " + player[i].name
			+ "	 " + std::to_string(static_cast<long long>(player[i].score));
		iText(SCREEN_WIDTH / 2 - 50, SCREEN_HEIGHT - 200 - 50*i, &texta[0], GLUT_BITMAP_TIMES_ROMAN_24);
		}
	}*/
...
void show_game_data(){
	...
	*/
	...
	int time_passed = frame_count/10;
	char timeText[20];
    _itoa_s(time_passed, timeText, sizeof(timeText), 10);
    std::string timeStr = "Time: " + std::string(timeText);
    iText(SCREEN_WIDTH - 200, SCREEN_HEIGHT - 120, &timeStr[0], GLUT_BITMAP_HELVETICA_18);
...
void update_positions(int value) {
 if (!isGamePaused && isGameOngoing ) {
     ...
     //   update_time();
        ...
        if (!isGamePaused && isGameOngoing && (currentMenu == LEVEL_1 || currentMenu == LEVEL_2 || currentMenu == LEVEL_3)){
			frame_count++;
		}
        //delay(0.02);
    }
